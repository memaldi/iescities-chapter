...
Taking into consideration the large amounts of data present at Smart Cities \index{{Smart City}}, data management's \index{{Data management}} complexity can be described in terms of:
\begin{itemize}
	\item Volume
	\item Variety
	\item Veracity
...
This four variables can also be found in \textit{Big Data}-related articles (also known as the \textit{Big Data's Vs}) \cite{zikopoulos2011understanding,russom2011big}, \index{{Big Data}} so it is not surprising at all that Smart Cities are going to deal with Big Data problems in the near future (if they are not dealing with them right now).
